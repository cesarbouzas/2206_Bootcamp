...
import com.ontimize.hr.model.core.dao.ExperienceDao;
 ...
 private void insertNonRelatedData(Map<String, Object> nonCandidateData) {
  for (Entry<String, Object> entry : nonCandidateData.entrySet()) {
   ...
   case CandidateDao.ATTR_EXPERIENCE_LEVEL:
    data.put(ExperienceDao.ATTR_DESCRIPTION, entry.getValue());
    attr.add(ExperienceDao.ATTR_ID);
    query = this.masterService.experienceQuery(data, attr);
    if (query.calculateRecordNumber() > 0) {
     entry.setValue(query.getRecordValues(0).get(ExperienceDao.ATTR_ID));
    } else {
     toret = this.masterService.experienceInsert(data);
     entry.setValue(toret.get(ExperienceDao.ATTR_ID));
    }
    break;
